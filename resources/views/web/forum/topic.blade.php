...
@if ($topic->threads()->count() == 0)
                <span>There are currently no forum topics. <a href="{{ route('forum.new', ['thread', $topic->id]) }}">Create one?</a></span>
            @else
...
@foreach ($topic->threads() as $thread)
					...
					<div class="grid-x grid-margin-x align-middle forum-topic-container">
						...
						<div class="large-3 cell text-right show-for-large" style="word-break:break-word;">
                           @if ($thread->lastReply())
							...
							<a href="'.$serverName.'/forum/thread/'.$getT->ID.'/'; if ($getT->LastPostReplyID != 0) { echo '#'.$getT->LastPostReplyID.''; } echo '" class="last-post-link">{{ $thread->title }}</a>
 ...
 {{ $topic->threads()->onEachSide(1)->links('vendor.pagination.aeo') }}
